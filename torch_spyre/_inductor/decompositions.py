...
def _prepare_kv(
    ...
    """Pad and, if needed, GQA-broadcast one of K/V, naming each real op.

    Each named_dims scope wraps EXACTLY ONE op whose output rank matches the
    name count.  A named_dims hint applies to every op traced in its scope and
    ``_named_dims`` registers sizes with setdefault, so a scope spanning ops of
    different shapes lets the first one win -- which is how an early version
    registered KV from a zero block's 64-row axis instead of the padded 320.
    The zero blocks are therefore built by ``_kv_pad_parts`` outside any scope.

    K and V are prepared SEPARATELY here.  An earlier version concatenated them
    along the head axis and sliced them back apart, to stop V's chain being
    ...
    if expansion != 1:
        # The expand MATERIALIZES (rank 5); the flatten after it is a view that
        # fuses downstream, so the rank-5 output is what there is to name.
        # unsqueeze stays outside the scope -- it is rank 5 too, but carries a 1
        # in the expansion slot, and setdefault would register EXP=1 from it.
        t5 = t.unsqueeze(2)
...
def _sliding_window_attention_looped(
    ...
    name count — see ``_prepare_kv``.  Violating that registered a dim's size
